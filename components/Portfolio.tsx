...
const Portfolio: React.FC<PortfolioProps> = ({ onNavigate }) => {
  return (
    <section id="portfolio" className="py-24 lg:py-32 bg-white px-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-16 lg:mb-20">
          <div className="max-w-xl">
            <motion.div 
              ...
              className="text-[#ff6b6b] font-bold text-[9px] uppercase tracking-[0.4em] mb-4 flex items-center gap-2"
            >
              <Sparkles size={12} />
              Recent Case Studies
            </motion.div>
            <h2 className="text-4xl lg:text-5xl font-bold text-[#121417] mb-5 tracking-tighter leading-none">
            ...
            <p className="text-base lg:text-lg text-[#6b7280] font-light leading-relaxed">
              Precision-engineered systems for education and high-growth commerce sectors.
            </p>
          </div>
          <motion.button 
            whileHover={{ x: 5 }}
            onClick={() => onNavigate('all-projects')}
            className="flex items-center gap-2 text-[#0fb9b1] font-bold text-[10px] uppercase tracking-[0.3em] group"
          >
            Archive 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
          {PROJECTS.map((project, index) => (
            <motion.div
              ...
              onClick={() => onNavigate('all-projects')}
              className="group relative rounded-[32px] lg:rounded-[48px] overflow-hidden cursor-pointer shadow-sm hover:shadow-2xl transition-all border border-gray-100 bg-[#fdfdfd]"
            >
              <div className="aspect-[16/10] overflow-hidden bg-gray-50">
                <img 
                  src={project.image} 
                  alt={project.title} 
                  className="w-full h-full object-cover img-enhanced group-hover:scale-105"
                />
              </div>
              ...
              <div className="p-8 lg:p-10 flex flex-col">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <span className="text-[#0fb9b1] font-bold text-[8px] uppercase tracking-[0.2em] mb-2 block">
                      {project.category}
                    </span>
